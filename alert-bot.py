...
def connect_database():
    ...
    conn = mysql.connector.connect(host=host, user=usr, password=passw, database=db)
    return conn
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await context.bot.send_message(
            ...
            text=f"Olá, {user.first_name}! Bem-vindo ao bot. Escolha um ambiente para continuar:\n\n/1 - Barragem\n/2 - Reservatórios",
        )
...
async def escolha_ambiente(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_choice == "/1":
        ...
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="Escolha o que deseja fazer entre os comandos relacionados aos Reservatórios:\n\n/A - Visualizar a última coleta de dados\n/B - Criar alerta\n/C - Visualizar alertas\n/D - Excluir alerta\n/Z - Voltar para o Menu",
        )
        return RESERVATORIOS

    else:
        await context.bot.send_message(
            chat_id=update.effective_chat.id, text="Escolha inválida."
        )
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="Escolha um ambiente para continuar:\n\n/1 - Barragem\n/2 - Reservatórios",
        )
        return ESCOLHA_AMBIENTE
...
async def barraA(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        cursor.execute(
            "SELECT CAST(sanep.sensor_data.data AS FLOAT) as Valor, sanep.sensors.name as Origem, sanep.sensor_data.created_at as Data FROM sanep.sensor_data LEFT JOIN sanep.sensors ON sanep.sensor_data.sensor_id = sanep.sensors.id WHERE sanep.sensors.type_id = 1 ORDER BY sanep.sensor_data.created_at DESC LIMIT 1"
        )
        ...
        # Executa uma consulta SELECT
        cursor.execute(
            "SELECT CAST(sanep.sensor_data.data AS FLOAT) as Valor, sanep.sensors.name as Origem, sanep.sensor_data.created_at as Data FROM sanep.sensor_data LEFT JOIN sanep.sensors ON sanep.sensor_data.sensor_id = sanep.sensors.id WHERE sanep.sensors.type_id = 2 ORDER BY sanep.sensor_data.created_at DESC LIMIT 1"
        )
        # Retorna os resultados
        ...
        # Executa uma consulta SELECT
        cursor.execute(
            "SELECT CAST(sanep.sensor_data.data AS FLOAT) as Valor, sanep.sensors.name as Origem, sanep.sensor_data.created_at as Data FROM sanep.sensor_data LEFT JOIN sanep.sensors ON sanep.sensor_data.sensor_id = sanep.sensors.id WHERE sanep.sensors.type_id = 6 ORDER BY sanep.sensor_data.created_at DESC LIMIT 1"
        )
        # Retorna os resultados
        ...
        print(mensagem)
        await context.bot.send_message(
            chat_id=update.effective_chat.id, text=f"{mensagem}"
        )
...
def salva_regra(sensor_sem_barra, condicao_sem_barra, valor_sem_barra):
    ...
    try:
        ...
        inserir_dados = (
            "INSERT INTO rules (sensor_id, `condition`, value) VALUES (%s, %s, %s)"
        )
...
async def barraB(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text=f"Adicionar alerta para qual sensor?\n\n/TEMPERATURA\n/UMIDADE\n/VENTO",
    )
    return SENSOR
...
async def receber_sensor(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sensor == "/TEMPERATURA" or sensor == "/UMIDADE" or sensor == "/VENTO":
        ...
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="Qual será a condição?\n\n/IGUAL\n/MENOR_QUE\n/MAIOR_QUE",
        )
        return CONDICAO
    else:
        await context.bot.send_message(
            chat_id=update.effective_chat.id, text="Desculpe, não entendi esse comando."
        )
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text=f"Adicionar alerta para qual sensor?\n\n/TEMPERATURA\n/UMIDADE\n/VENTO",
...
async def receber_condicao(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if condicao == "/IGUAL" or condicao == "/MENOR_QUE" or condicao == "/MAIOR_QUE":
        ...
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="Digite o valor de referência.\nPor exemplo: /23.",
        )
        return VALOR
    else:
        await context.bot.send_message(
...
async def receber_valor(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if re.match(r"^/\d+", valor):
        ...
        condicao = context.user_data[
            "condicao"
        ]  # Recupera a condição da memória do bot
        ...
        await context.bot.send_message(
            chat_id=update.effective_chat.id, text="Alerta criado!"
        )
...
async def barraC(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await context.bot.send_document(
            chat_id=update.effective_chat.id, document=InputFile(open(pdf_path, "rb"))
        )
...
async def barraD(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await context.bot.send_message(
        chat_id=update.effective_chat.id,
        text=f"Digite o ID do alerta que deseja excluir.\nPor exemplo: /23.",
    )
    ...
    if re.match(r"^/\d+", id_delete):
        ...
        try:
            ...
            if cursor.rowcount == 1:
                ...
                await context.bot.send_message(
                    chat_id=update.effective_chat.id, text="Alerta excluído!"
                )
                return ConversationHandler.END  # Encerra a conversa
            else:
                ...
                await context.bot.send_message(
                    chat_id=update.effective_chat.id,
                    text=f"Nenhum alerta com ID = {id_delete_sem_barra} foi encontrado.",
                )
                return ConversationHandler.END  # Encerra a conversa
        ...
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text="Para excluir um alerta, digite /3 novamente.",
        )
        return ConversationHandler.END  # Encerra a conversa
                ...
                if row[2] == "IGUAL":
                    if temperatura[0][0] == row[3]:
                        for i in telegram_users:
                            ...
                            await context.bot.send_message(
                                chat_id=user_id, text=f"{mensagem}"
                            )
...
if __name__ == "__main__":
    # Cria uma instância de um construtor de aplicativos.
    ...
    criaAlerta = ConversationHandler(
        entry_points=[CommandHandler("B", barraB)],
        states={
            SENSOR: [MessageHandler(filters.TEXT & filters.COMMAND, receber_sensor)],
            CONDICAO: [
                MessageHandler(filters.TEXT & filters.COMMAND, receber_condicao)
            ],
    ...
    excluirAlerta = ConversationHandler(
        entry_points=[CommandHandler("D", barraD)],
        states={
            EXCLUIR_ALERTA: [
                MessageHandler(filters.TEXT & filters.COMMAND, excluir_alerta)
            ],
        },
        fallbacks=[],
    )
    ...
    application.add_handler(
        ConversationHandler(
            ...
            states={
                ESCOLHA_AMBIENTE: [
                    MessageHandler(filters.TEXT or filters.COMMAND, escolha_ambiente)
                ],
                BARRAGEM: [
                    CommandHandler("A", barraA),
                    criaAlerta,
